#!/usr/bin/env python3
"""Complete Stacker News signup - click email to view magic code."""
import asyncio
import re
...
async def signup_stacker():
    async with async_playwright() as p:
        ...
        # Wait and check Guerrilla Mail for magic code
        ...
        for attempt in range(12):
            ...
            # Get page HTML to find email links
            html = await page.content()
            
            # Look for stacker email links
            if 'stacker' in html.lower():
                print(f"Attempt {attempt+1}: Found 'stacker' in HTML")
                
                # Find email rows/links
                email_rows = await page.query_selector_all('tr, [class*="mail"], [class*="email"], [class*="message"]')
                for row in email_rows[:30]:
                    text = await row.inner_text()
                    if 'stacker' in text.lower() or 'magic' in text.lower() or 'code' in text.lower() or 'verify' in text.lower():
                        print(f"  Found email row: {text[:200]}")
                        # Try to click the row or find link
                        try:
                            await row.click()
                            await page.wait_for_timeout(3000)
                            email_body = await page.inner_text('body')
                            print(f"  Email body: {email_body[:3000]}")
                        ...
                        # Also try links in row
                        links = await row.query_selector_all('a')
                        for link in links:
                            href = await link.get_attribute('href')
                            if href:
                                try:
                                    full_url = href if href.startswith('http') else f"https://www.guerrillamail.com{href}"
                                    await page.goto(full_url, wait_until="networkidle")
                                    await page.wait_for_timeout(3000)
                                    email_body = await page.inner_text('body')
                                    print(f"  Email via link: {email_body[:3000]}")
                                    codes = re.findall(r'\b\d{6}\b', email_body)
                                    for code in codes:
                                        if code != '000000':
                                            magic_code = code
                ...
                print(f"Attempt {attempt+1}: No stacker email yet")
