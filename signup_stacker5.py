#!/usr/bin/env python3
"""Complete Stacker News signup - extract magic code from inbox text."""
import asyncio
import re
...
async def signup_stacker():
    async with async_playwright() as p:
        ...
        for attempt in range(12):  # 12 attempts * 5 seconds = 60 seconds
            ...
            # Look for stacker email - extract 6-digit codes from entire page
            codes = re.findall(r'\b\d{6}\b', body_text)
            if codes:
                # Filter for likely magic codes (not timestamps like 00:23:22)
                for code in codes:
                    if code != '000000' and len(code) == 6:
                        magic_code = code
                        print(f"Attempt {attempt+1}: Found potential magic code: {magic_code}")
                        break
                if magic_code:
                    break
            
            # Also search for stacker keyword
            if 'stacker' in body_text.lower():
                print(f"Attempt {attempt+1}: Found 'stacker' in inbox")
                codes = re.findall(r'\b\d{6}\b', body_text)
                for code in codes:
                    if code != '000000':
                        magic_code = code
                        print(f"Found magic code near 'stacker': {magic_code}")
                        break
                if magic_code:
                    break
            
            print(f"Attempt {attempt+1}: No magic code yet, waiting...")
